...
context('Verify that web app works correctly', () => {
    before(() => {
      ...
      it("Check the parties menu is correct",()=>{
        cy.get('.col-lg-3 > .Navigation > ul > :nth-child(3) > :nth-child(1) > .inactive > .text',{timeout:500}).click()
        cy.location('pathname').should('include','parties')
        cy.go('back')
       
      })
      it("Check the how_to_vote menu is correct",()=>{
        cy.get('.col-lg-3 > .Navigation > ul > :nth-child(4) > :nth-child(1) > .inactive > .text',{timeout:500}).click()
        cy.location('pathname').should('include','vote')
        cy.go('back')
      })
      it("Check the faq menu is correct",()=>{
        cy.get('.col-lg-3 > .Navigation > ul > :nth-child(5) > :nth-child(1) > .inactive > .text',{timeout:500}).click()
      ...
      it.only("Navigate the candidates list by filtering location",()=>{
